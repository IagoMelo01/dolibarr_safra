...
/**
 *   	\file       colheita_list.php
 *		\ingroup    safra
 *		\brief      List page for colheita
 */
...
require_once __DIR__.'/class/colheita.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'colheita', 'write');
	$permissiontodelete = $user->hasRight('safra', 'colheita', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Colheita';
	$objectlabel = 'Colheita';
...
$title = $langs->trans("Colheitas");
//$help_url = "EN:Module_Colheita|FR:Module_Colheita_FR|ES:Módulo_Colheita";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/colheita_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/colheita_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendColheitaRef";
$modelmail = "colheita";
$objecttmp = new Colheita($db);
