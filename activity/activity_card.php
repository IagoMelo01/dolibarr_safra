...
print '<style>
    :root {
        --safra-ink: #0f172a;
        --safra-muted: #475569;
        --safra-surface: #f8fafc;
        --safra-stroke: #e2e8f0;
        --safra-primary: #2563eb;
        --safra-primary-soft: #e0e7ff;
        --safra-accent: #22c55e;
    }
    .safra-activity-card {
        border-radius: 18px;
        overflow: hidden;
        background: #fff;
        border: 1px solid var(--safra-stroke);
    ...
    .safra-activity-card .form-control:focus, .safra-activity-card select:focus {
        border-color: var(--safra-primary);
    ...
    .safra-activity-card .mixture-modal.is-visible {
        display: flex;
    }
    ...
    .safra-activity-card .mixture-modal.is-visible .modal-dialog {
    ...
    .safra-activity-card .mixture-modal .modal-footer {
        border-top: 1px solid var(--safra-stroke);
        background: #f8fafc;
    }
    ...
    .safra-activity-card .mixture-summary .text-muted {
        padding: 0.5rem 0.75rem;
        border-radius: 10px;
        background: #fff;
        border: 1px dashed var(--safra-stroke);
    }
    body.safra-modal-open {
        overflow: hidden;
    }
</style>';
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close btn-close-white close-mixture" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-footer border-0 d-flex justify-content-between">
                <button type="button" class="btn btn-outline-secondary close-mixture"><?php echo $langs->trans('Cancel'); ?></button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showMixtureModal() {
        if (!mixtureModal) return;
        mixtureModal.classList.add('is-visible');
        document.body.classList.add('safra-modal-open');
    }
    function hideMixtureModal() {
        if (!mixtureModal) return;
        mixtureModal.classList.remove('is-visible');
        document.body.classList.remove('safra-modal-open');
    }

    var mixtureButton = document.getElementById('open-mixture');
    if (mixtureButton && mixtureModal) {
    ...
    if (mixtureModal) {
        mixtureModal.addEventListener('click', function (e) {
            if (e.target === mixtureModal) {
                hideMixtureModal();
            }
        });
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape' && mixtureModal.classList.contains('is-visible')) {
                hideMixtureModal();
            }
        });
        var closeButtons = mixtureModal.querySelectorAll('.close-mixture');
        closeButtons.forEach(function (btn) {
            btn.addEventListener('click', hideMixtureModal);
        });
    }
    ...
    var saveMixture = document.getElementById('save-mixture-note');
    if (saveMixture) {
        saveMixture.addEventListener('click', function () {
