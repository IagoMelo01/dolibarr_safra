...
/**
 *   	\file       analisesolo_list.php
 *		\ingroup    safra
 *		\brief      List page for analisesolo
 */
...
require_once __DIR__.'/class/analisesolo.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'analisesolo', 'write');
	$permissiontodelete = $user->hasRight('safra', 'analisesolo', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'AnaliseSolo';
	$objectlabel = 'AnaliseSolo';
...
$title = $langs->trans("AnaliseSolos");
//$help_url = "EN:Module_AnaliseSolo|FR:Module_AnaliseSolo_FR|ES:Módulo_AnaliseSolo";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/analisesolo_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/analisesolo_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAnaliseSoloRef";
$modelmail = "analisesolo";
$objecttmp = new AnaliseSolo($db);
