...
function safra_talhao_parse_kml($kmlContent, array &$errors = array())
{
        ...
        $prefix = '';
        if (!empty($namespaces['kml'])) {
                $xml->registerXPathNamespace('kml', $namespaces['kml']);
                $prefix = 'kml:';
        }
        ...
        foreach ($placemarks as $placemark) {
                ...
                $properties = safra_talhao_kml_extract_properties($placemark, $prefix);
                $polygons = safra_talhao_kml_extract_polygons($placemark, $prefix);
 ...
 * @param string           $prefix    Namespace prefix including trailing ':' when present
 * @return array<int,array<int,array{0:float,1:float}>>
 */
function safra_talhao_kml_extract_polygons($placemark, $prefix = '')
{
        $polygons = array();

        $polygonNodes = $placemark->xpath('.//'.$prefix.'Polygon');
        if ($polygonNodes === false) {
                $polygonNodes = array();
        }

        foreach ($polygonNodes as $polygon) {
                $coordNodes = $polygon->xpath('.//'.$prefix.'outerBoundaryIs/'.$prefix.'LinearRing/'.$prefix.'coordinates');
                if ($coordNodes === false) {
                        $coordNodes = array();
                }

                foreach ($coordNodes as $coordNode) {
                        $coords = safra_talhao_parse_kml_coordinates((string) $coordNode);
                        if (count($coords) >= 3) {
                                $polygons[] = $coords;
                        }
                }
 ...
 */
function safra_talhao_kml_extract_properties($placemark, $prefix = '')
{
        $properties = array();

        $dataNodes = $placemark->xpath('.//'.$prefix.'ExtendedData/'.$prefix.'Data');
        if ($dataNodes !== false) {
                foreach ($dataNodes as $node) {
                        $name = (string) $node['name'];
                        $value = trim((string) $node->{$prefix.'value'});
                        if ($value === '' && isset($node->value)) {
        ...
        $simpleDataNodes = $placemark->xpath('.//'.$prefix.'SimpleData');
