...
$migrationFile = $root . '/sql/migrations/20260504_rebuild_activity_schema.sql';
$upgradeFile = $root . '/upgrade.php';
$mysqlSchemaFile = $root . '/sql/mysql/activity.sql';
$llxSchemaFile = $root . '/sql/llx_safra_activity.sql';

$assert(is_file($migrationFile), 'Missing migration file 20260504_rebuild_activity_schema.sql');
...
$assert(stripos($upgradeContent, '20260504_rebuild_activity_schema.sql') !== false, 'upgrade.php must execute current rebuild migration');
$assert(stripos($upgradeContent, '$migrationOk = safraMigrateLegacyAplicacao') === false, 'upgrade.php must not call legacy activity migration during rebuild');
