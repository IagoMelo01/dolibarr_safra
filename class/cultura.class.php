...
class Cultura extends CommonObject
{
	...
	public $status;
	public $embrapa_id;
	public $necessidade_n;
	public $necessidade_p;
	public $necessidade_k;
	public $saturacao_bases_ideal;
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Cultura', 'fk_element'=>'fk_cultura'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'cultura', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'cultura', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'cultura_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'cultura/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'cultura/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowCultura")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Cultura").'</u>';
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_cultura_standard';
		}
