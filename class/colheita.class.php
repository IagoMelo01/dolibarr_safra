...
/**
 * \file        class/colheita.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for Colheita (Create/Read/Update/Delete)
 */
 ...
 * Class for Colheita
 */
class Colheita extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'colheita';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_colheita';
	...
	/**
	 * @var string String with name of icon for colheita. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'colheita@safra' if picto is file 'img/object_colheita.png'.
	 */
	...
	public $fields=array(
		...
		"quantidade_colhida" => array("type"=>"double(28,4)", "label"=>"Quantidade Colhida(kg)", "enabled"=>"1", 'position'=>50, 'notnull'=>1, "visible"=>"1", "isameasure"=>"1",),
		"data_colheita" => array("type"=>"date", "label"=>"Data da Colheita", "enabled"=>"1", 'position'=>50, 'notnull'=>1, "visible"=>"1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'safra_colheitaline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_colheita';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Colheitaline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Colheita', 'fk_element'=>'fk_colheita'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_colheitadet');

	// /**
	//  * @var ColheitaLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'colheita', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'colheita', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'colheita_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'colheita/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'colheita/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'colheita/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'colheita/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/colheita/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/colheita/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/colheita/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowColheita")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Colheita").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/colheita_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowColheita");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ColheitaLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_colheita = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_colheita_standard';
		}
...
/**
 * Class ColheitaLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ColheitaLine extends CommonObjectLine
{
	// To complete with content of an object ColheitaLine
	// We should have a field rowid, fk_colheita and position
