...
/**
 * \file        class/cultivar.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for Cultivar (Create/Read/Update/Delete)
 */
 ...
 * Class for Cultivar
 */
class Cultivar extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'cultivar';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_cultivar';
	...
	/**
	 * @var string String with name of icon for cultivar. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'cultivar@safra' if picto is file 'img/object_cultivar.png'.
	 */
	...
	public $status;
	public $cultura;
	public $obtentor_mantenedor;
	public $rnc;
	public $embrapa_id;
	public $safra;
	public $potencial_produtivo;
	public $uf;
	public $grupo;
	public $floracao;
	public $maturacao_fisiologica;
	public $enchimento_graos;
	public $sistema_cultivo;
	public $genetica;
	public $regiao;
	...
	//  */
	// public $table_element_line = 'safra_cultivarline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_cultivar';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Cultivarline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Cultivar', 'fk_element'=>'fk_cultivar'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_cultivardet');

	// /**
	//  * @var CultivarLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'cultivar', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'cultivar', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'cultivar_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'cultivar/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'cultivar/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'cultivar/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'cultivar/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/cultivar/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/cultivar/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/cultivar/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowCultivar")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Cultivar").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/cultivar_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowCultivar");
	...
	public function getLinesArray()
	{
		...
		$objectline = new CultivarLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_cultivar = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_cultivar_standard';
		}
...
/**
 * Class CultivarLine. You can also remove this and generate a CRUD class for lines objects.
 */
class CultivarLine extends CommonObjectLine
{
	// To complete with content of an object CultivarLine
	// We should have a field rowid, fk_cultivar and position
