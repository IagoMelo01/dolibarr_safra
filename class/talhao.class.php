...
class Talhao extends CommonObject
{
	...
	public $fields=array(
		...
		"area" => array("type"=>"double(28,4)", "label"=>"Área (ha) ", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1", "isameasure"=>"1",),
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Talhao', 'fk_element'=>'fk_talhao'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'talhao', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'talhao', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'talhao_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'talhao/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'talhao/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTalhao")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Talhao").'</u>';
	...
	public function getKanbanView($option = '', $arraydata = null)
	{
		...
		if (property_exists($this, 'area')) {
			$return .= '<br>';
			$return .= '<span class="info-box-label area">'.round($this->area, 2).' Ha</span>';
		}
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_talhao_standard';
		}
