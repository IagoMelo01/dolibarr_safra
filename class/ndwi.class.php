...
/**
 * \file        class/ndwi.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for NDWI (Create/Read/Update/Delete)
 */
 ...
 * Class for NDWI
 */
class NDWI extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'safra_ndwi';
	...
	/**
	 * @var string String with name of icon for ndwi. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'ndwi@safra' if picto is file 'img/object_ndwi.png'.
	 */
	...
	public $fields=array(
		...
		"caminho_json" => array("type"=>"text", "label"=>"caminho_json", "enabled"=>"1", 'position'=>50, 'notnull'=>1, "visible"=>"1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'safra_ndwiline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_ndwi';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'NDWIline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'NDWI', 'fk_element'=>'fk_ndwi'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_ndwidet');

	// /**
	//  * @var NDWILine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'ndwi', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'ndwi', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'ndwi_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'ndwi/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'ndwi/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'ndwi/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'ndwi/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/ndwi/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/ndwi/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/ndwi/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowNDWI")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("NDWI").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/ndwi_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new NDWILine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_ndwi = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_ndwi_standard';
		}
	...
	/**
	 * Regist the ndwi data from a specific talhao or the latest ndwi data from all talhaos
	 *
	 ...
	 * @return void
	 */
	public function requestNDWIData(User $user = null, $time = null, Talhao $talhao = null)
	{
		...
		foreach($talhao as $key){
			...
			$params = array(
				'SERVICE' => 'WMS',
				'REQUEST' => 'GetMap',
				'LAYERS' => 'NDWI',  // Substitua por sua camada de dados configurada
				'TRANSPARENT' => 'true',
			...
			$file_path = DOL_DOCUMENT_ROOT.'/custom/safra/json/ndwi/'. $file_name .'.json'; // Altere para o diretório desejado
...
/**
 * Class NDWILine. You can also remove this and generate a CRUD class for lines objects.
 */
class NDWILine extends CommonObjectLine
{
	// To complete with content of an object NDWILine
	// We should have a field rowid, fk_ndwi and position
