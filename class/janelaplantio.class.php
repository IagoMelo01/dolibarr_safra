...
/**
 * \file        class/janelaplantio.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for JanelaPlantio (Create/Read/Update/Delete)
 */
...
/**
 * Class for JanelaPlantio
 */
class JanelaPlantio extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'janelaplantio';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_janelaplantio';
	...
	/**
	 * @var string String with name of icon for janelaplantio. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'janelaplantio@safra' if picto is file 'img/object_janelaplantio.png'.
	 */
	...
	public $fields=array(
		...
		"municipio" => array("type"=>"integer:Talhao:safra/class/talhao.class.php", "label"=>"Município", "enabled"=>"1", 'position'=>50, 'notnull'=>1, "visible"=>"1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'safra_janelaplantioline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_janelaplantio';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'JanelaPlantioline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'JanelaPlantio', 'fk_element'=>'fk_janelaplantio'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_janelaplantiodet');

	// /**
	//  * @var JanelaPlantioLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'janelaplantio', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'janelaplantio', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'janelaplantio_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'janelaplantio/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'janelaplantio/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'janelaplantio/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'janelaplantio/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/janelaplantio/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/janelaplantio/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/janelaplantio/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowJanelaPlantio")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("JanelaPlantio").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/janelaplantio_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowJanelaPlantio");
	...
	public function getLinesArray()
	{
		...
		$objectline = new JanelaPlantioLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_janelaplantio = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_janelaplantio_standard';
		}
...
/**
 * Class JanelaPlantioLine. You can also remove this and generate a CRUD class for lines objects.
 */
class JanelaPlantioLine extends CommonObjectLine
{
	// To complete with content of an object JanelaPlantioLine
	// We should have a field rowid, fk_janelaplantio and position
