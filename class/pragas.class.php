...
/**
 * \file        class/pragas.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for Pragas (Create/Read/Update/Delete)
 */
 ...
 * Class for Pragas
 */
class Pragas extends CommonObject
{
	/**
	...
	/**
	 * @var string 	If permission must be checkec with hasRight('safra', 'read') and not hasright('mymodyle', 'pragas', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'safra';

	/**
	 * @var string 	String with name of icon for pragas. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'pragas@safra' if picto is file 'img/object_pragas.png'.
	 */
	public $picto = 'fa-bug';
	...
	public $fields = array(
		...
		"label" => array("type" => "varchar(255)", "label" => "Nome Comum", "enabled" => "1", 'position' => 30, 'notnull' => 0, "visible" => "1", "searchall" => "1", "css" => "minwidth300", "cssview" => "wordbreak", "help" => "Help text", "validate" => "1",),
		...
		"label_cientifico" => array("type" => "text", "label" => "Nome Científico", "enabled" => "1", 'position' => 31, 'notnull' => 0, "visible" => "1",),
		"culturas" => array("type" => "text", "label" => "culturas", "enabled" => "1", 'position' => 32, 'notnull' => 0, "visible" => "1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'safra_pragasline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_pragas';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Pragasline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Pragas', 'fk_element'=>'fk_pragas'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_pragasdet');

	// /**
	//  * @var PragasLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'pragas', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'pragas', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'pragas_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'pragas/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'pragas/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'pragas/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'pragas/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/pragas/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/pragas/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/pragas/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPragas")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Pragas").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/pragas_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new PragasLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_pragas:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_pragas_standard';
		}
		...
		if (getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefPragas $obj';
...
/**
 * Class PragasLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PragasLine extends CommonObjectLine
{
	// To complete with content of an object PragasLine
	// We should have a field rowid, fk_pragas and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'pragas'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_pragas'
