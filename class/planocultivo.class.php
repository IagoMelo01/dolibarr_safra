...
/**
 * \file        class/planocultivo.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for PlanoCultivo (Create/Read/Update/Delete)
 */
...
/**
 * Class for PlanoCultivo
 */
class PlanoCultivo extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'planocultivo';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_planocultivo';
	...
	/**
	 * @var string String with name of icon for planocultivo. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'planocultivo@safra' if picto is file 'img/object_planocultivo.png'.
	 */
	public $picto = 'fa-map';
	...
	//  */
	// public $table_element_line = 'safra_planocultivoline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_planocultivo';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'PlanoCultivoline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'PlanoCultivo', 'fk_element'=>'fk_planocultivo'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_planocultivodet');

	// /**
	//  * @var PlanoCultivoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'planocultivo', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'planocultivo', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'planocultivo_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'planocultivo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'planocultivo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'planocultivo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'planocultivo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/planocultivo/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/planocultivo/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/planocultivo/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPlanoCultivo")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("PlanoCultivo").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/planocultivo_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowPlanoCultivo");
	...
	public function getLinesArray()
	{
		...
		$objectline = new PlanoCultivoLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_planocultivo = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_planocultivo_standard';
		}
...
/**
 * Class PlanoCultivoLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PlanoCultivoLine extends CommonObjectLine
{
	// To complete with content of an object PlanoCultivoLine
	// We should have a field rowid, fk_planocultivo and position
