...
class Municipio extends CommonObject
{
	...
	public $fields=array(
		...
		"data_atualizacao" => array("type"=>"varchar(128)", "label"=>"Data de atualização", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"3",),
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Municipio', 'fk_element'=>'fk_municipio'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'municipio', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'municipio', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'municipio_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'municipio/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'municipio/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowMunicipio")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Municipio").'</u>';
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_municipio_standard';
		}
