...
/**
 * \file        class/evi.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for EVI (Create/Read/Update/Delete)
 */
 ...
 * Class for EVI
 */
class EVI extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'safra_evi';
	...
	/**
	 * @var string String with name of icon for evi. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'evi@safra' if picto is file 'img/object_evi.png'.
	 */
	...
	//  */
	// public $table_element_line = 'safra_eviline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_evi';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'EVIline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'EVI', 'fk_element'=>'fk_evi'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_evidet');

	// /**
	//  * @var EVILine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'evi', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'evi', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'evi_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'evi/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'evi/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'evi/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'evi/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/evi/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/evi/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/evi/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowEVI")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("EVI").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/evi_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new EVILine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_evi = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_evi_standard';
		}
	...
	public function requestEVIData(User $user = null, $time = null, Talhao $talhao = null)
	{
		...
		foreach($talhao as $key){
			...
			$file_path = DOL_DOCUMENT_ROOT.'/custom/safra/json/evi/'. $file_name .'.json'; // Altere para o diretÃ³rio desejado
...
/**
 * Class EVILine. You can also remove this and generate a CRUD class for lines objects.
 */
class EVILine extends CommonObjectLine
{
	// To complete with content of an object EVILine
	// We should have a field rowid, fk_evi and position
