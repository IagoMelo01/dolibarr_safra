...
class RecomendacaoAdubo extends CommonObject
{
	...
	public $fields=array(
		...
		"plano_cultivo" => array("type"=>"integer:planocultivo:safra/class/planocultivo.class.php", "label"=>"Plano de Cultivo", "enabled"=>"1", 'position'=>50, 'notnull'=>1, "visible"=>"1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'RecomendacaoAdubo', 'fk_element'=>'fk_recomendacaoadubo'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'recomendacaoadubo', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'recomendacaoadubo', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'recomendacaoadubo_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'recomendacaoadubo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'recomendacaoadubo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowRecomendacaoAdubo")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("RecomendacaoAdubo").'</u>';
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_recomendacaoadubo_standard';
		}
