...
/**
 * \file        class/swir.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for SWIR (Create/Read/Update/Delete)
 */
 ...
 * Class for SWIR
 */
class SWIR extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'safra_swir';
	...
	/**
	 * @var string String with name of icon for swir. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'swir@safra' if picto is file 'img/object_swir.png'.
	 */
	...
	//  */
	// public $table_element_line = 'safra_swirline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_swir';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'SWIRline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'SWIR', 'fk_element'=>'fk_swir'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_swirdet');

	// /**
	//  * @var SWIRLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'swir', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'swir', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'swir_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'swir/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'swir/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'swir/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'swir/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/swir/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/swir/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/swir/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowSWIR")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("SWIR").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/swir_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new SWIRLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_swir = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_swir_standard';
		}
	...
	/**
	 * Regist the swir data from a specific talhao or the latest swir data from all talhaos
	 *
	 ...
	 * @return void
	 */
	public function requestSWIRData(User $user = null, $time = null, Talhao $talhao = null)
	{
		...
		foreach($talhao as $key){
			...
			$params = array(
				'SERVICE' => 'WMS',
				'REQUEST' => 'GetMap',
				'LAYERS' => 'SWIR',  // Substitua por sua camada de dados configurada
				'TRANSPARENT' => 'true',
			...
			$file_path = DOL_DOCUMENT_ROOT.'/custom/safra/json/swir/'. $file_name .'.json'; // Altere para o diretÃ³rio desejado
			...
			$swir->label = $time;
			$swir->talhao = $key->id;
			$swir->date_creation = dol_now();
			$swir->caminho_json = './json/swir/'. $file_name .'.json';
			$swir->create($user);
...
/**
 * Class SWIRLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SWIRLine extends CommonObjectLine
{
	// To complete with content of an object SWIRLine
	// We should have a field rowid, fk_swir and position
