...
/**
 * \file        class/zoneamento.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for Zoneamento (Create/Read/Update/Delete)
 */
...
/**
 * Class for Zoneamento
 */
class Zoneamento extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'zoneamento';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_zoneamento';
	...
	/**
	 * @var string String with name of icon for zoneamento. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'zoneamento@safra' if picto is file 'img/object_zoneamento.png'.
	 */
	...
	public $fields=array(
		...
		"municipio" => array("type"=>"varchar(255)", "label"=>"Município", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"uf" => array("type"=>"varchar(4)", "label"=>"UF", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"cultura" => array("type"=>"varchar(128)", "label"=>"Cultura", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"ciclo" => array("type"=>"varchar(255)", "label"=>"Ciclo", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"dia_ini" => array("type"=>"integer", "label"=>"Dia Início", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"mes_ini" => array("type"=>"integer", "label"=>"Mês Início", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"dia_fim" => array("type"=>"integer", "label"=>"Dia fim", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"mes_fim" => array("type"=>"integer", "label"=>"Mês Fim", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
		"safra_ini" => array("type"=>"integer", "label"=>"Safra Início", "enabled"=>"1", 'position'=>50, 'notnull'=>0, "visible"=>"1",),
	...
	//  */
	// public $table_element_line = 'safra_zoneamentoline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_zoneamento';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Zoneamentoline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Zoneamento', 'fk_element'=>'fk_zoneamento'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_zoneamentodet');

	// /**
	//  * @var ZoneamentoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'zoneamento', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'zoneamento_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'zoneamento/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'zoneamento/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'zoneamento/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'zoneamento/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/zoneamento/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/zoneamento/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/zoneamento/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowZoneamento")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Zoneamento").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/zoneamento_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowZoneamento");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ZoneamentoLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_zoneamento = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_zoneamento_standard';
		}
...
/**
 * Class ZoneamentoLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ZoneamentoLine extends CommonObjectLine
{
	// To complete with content of an object ZoneamentoLine
	// We should have a field rowid, fk_zoneamento and position
