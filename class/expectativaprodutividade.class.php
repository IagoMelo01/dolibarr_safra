...
/**
 * \file        class/expectativaprodutividade.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for ExpectativaProdutividade (Create/Read/Update/Delete)
 */
...
/**
 * Class for ExpectativaProdutividade
 */
class ExpectativaProdutividade extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'expectativaprodutividade';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_expectativaprodutividade';
	...
	/**
	 * @var string String with name of icon for expectativaprodutividade. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'expectativaprodutividade@safra' if picto is file 'img/object_expectativaprodutividade.png'.
	 */
	...
	//  */
	// public $table_element_line = 'safra_expectativaprodutividadeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_expectativaprodutividade';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'ExpectativaProdutividadeline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'ExpectativaProdutividade', 'fk_element'=>'fk_expectativaprodutividade'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_expectativaprodutividadedet');

	// /**
	//  * @var ExpectativaProdutividadeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'expectativaprodutividade', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'expectativaprodutividade_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'expectativaprodutividade/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'expectativaprodutividade/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'expectativaprodutividade/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'expectativaprodutividade/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/expectativaprodutividade/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/expectativaprodutividade/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/expectativaprodutividade/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowExpectativaProdutividade")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("ExpectativaProdutividade").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/expectativaprodutividade_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowExpectativaProdutividade");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ExpectativaProdutividadeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_expectativaprodutividade = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_expectativaprodutividade_standard';
		}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_expectativaprodutividade';
...
/**
 * Class ExpectativaProdutividadeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ExpectativaProdutividadeLine extends CommonObjectLine
{
	// To complete with content of an object ExpectativaProdutividadeLine
	// We should have a field rowid, fk_expectativaprodutividade and position
