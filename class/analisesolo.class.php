...
/**
 * \file        class/analisesolo.class.php
 * \ingroup     safra
 * \brief       This file is a CRUD class file for AnaliseSolo (Create/Read/Update/Delete)
 */
...
/**
 * Class for AnaliseSolo
 */
class AnaliseSolo extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'analisesolo';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'safra_analisesolo';
	...
	/**
	 * @var string String with name of icon for analisesolo. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'analisesolo@safra' if picto is file 'img/object_analisesolo.png'.
	 */
	public $picto = 'fa-chart-pie';
	...
	//  */
	// public $table_element_line = 'safra_analisesololine';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_analisesolo';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'AnaliseSololine';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'AnaliseSolo', 'fk_element'=>'fk_analisesolo'));
	...
	//  */
	// protected $childtablesoncascade = array('safra_analisesolodet');

	// /**
	//  * @var AnaliseSoloLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('safra', 'analisesolo', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'analisesolo', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('safra', 'analisesolo_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'analisesolo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'analisesolo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'analisesolo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'analisesolo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->safra->dir_output.'/analisesolo/'.$oldref;
				$dirdest = $conf->safra->dir_output.'/analisesolo/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->safra->dir_output.'/analisesolo/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowAnaliseSolo")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("AnaliseSolo").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/safra/analisesolo_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowAnaliseSolo");
	...
	public function getLinesArray()
	{
		...
		$objectline = new AnaliseSoloLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_analisesolo = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('SAFRA_MYOBJECT_ADDON')) {
			$conf->global->SAFRA_MYOBJECT_ADDON = 'mod_analisesolo_standard';
		}
...
/**
 * Class AnaliseSoloLine. You can also remove this and generate a CRUD class for lines objects.
 */
class AnaliseSoloLine extends CommonObjectLine
{
	// To complete with content of an object AnaliseSoloLine
	// We should have a field rowid, fk_analisesolo and position
