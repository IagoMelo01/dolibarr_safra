...
/**
 *  \file       aplicacao_document.php
 *  \ingroup    safra
 *  \brief      Tab for documents linked to Aplicacao
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->safra->multidir_output[$object->entity ? $object->entity : $conf->entity]."/aplicacao/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('safra', 'aplicacao', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Aplicacao")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Aplicacao"), -1, $object->picto);
...
$modulepart = 'safra';
$param = '&id='.$object->id;
//$relativepathwithnofile='aplicacao/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'aplicacao/'.dol_sanitizeFileName($object->ref).'/';
