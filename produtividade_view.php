...
if ($idCultura > 0) {
    ...
    $cultivares = $cultivarDao->fetchAll('ASC', 'label', 0, 0, $cultivarFilter);
}
                            ...
                            <?php } else { ?>
                                ...
                                if (is_array($cultivares) && !empty($cultivares)) {
                                    foreach ($cultivares as $cultivar) {
                                        ...
                                        $embrapaAttr = empty($cultivar->embrapa_id) ? '' : ' data-embrapa="' . dol_escape_htmltag($cultivar->embrapa_id) . '"';
                                        echo '<option value="' . (int) $cultivar->id . '" data-cultura="' . (int) $cultivar->cultura . '"' . $embrapaAttr . $selected . '>' . dol_escape_htmltag($label) . '</option>';
                                    }
                                } else {
