...
/**
 *   	\file       planocultivo_list.php
 *		\ingroup    safra
 *		\brief      List page for planocultivo
 */
...
require_once __DIR__.'/class/planocultivo.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('safra', 'planocultivo', 'write');
	$permissiontodelete = $user->hasRight('safra', 'planocultivo', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PlanoCultivo';
	$objectlabel = 'PlanoCultivo';
...
$title = $langs->trans("PlanoCultivos");
//$help_url = "EN:Module_PlanoCultivo|FR:Module_PlanoCultivo_FR|ES:Módulo_PlanoCultivo";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/safra/planocultivo_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/safra/planocultivo_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPlanoCultivoRef";
$modelmail = "planocultivo";
$objecttmp = new PlanoCultivo($db);
